...
#
# 레버 = 줄-완성 조각 우선 배급(주) + 밴드 완화(3패) + 비행기 배급(보너스). 기각된 레버 셋:
...
#     ⚠S8 재검토: 이 기각은 **판에 따라 다르다.** 클라이맥스(거점사 지배)에선 막힘 비용이
#       +2.4pt뿐이고 승률이 +11pt 오른다. 그래도 안 쓴다 — 아래 밴드 완화가 같은 사인을
#       네 배로(+42pt) 치면서 팡팡까지 사기 때문이다.
#   ✗ 전진 완화(위협의 전진 주기 +1배치) — S8에서 배선했다가 밴드에 밀려 걷어냈다.
#     ⚠기각 사유가 '안 먹혀서'가 아니다. 클라이맥스 43%→77.4%로 잘 먹혔고, 플테에서
#       **속도 차이를 못 느꼈다**(= 안 들킨다)는 판정까지 받았다. 진 이유는 옆 레버가 더 나아서다:
#         전진 완화  77.4% · 배치 56.6(+16%) · 처치/클리어 2.13 (거의 무변화)
#         밴드 완화  85.0% · 배치 49.2(+0.6%) · 처치/클리어 2.54 (+23%)
#       더 이기게 하면서 **판을 안 늘리고**, 승률 말고 '팡팡'까지 산다. 둘 다 켜면 96.7% = 과함.
#     ⇒ 되살릴 자리: 밴드가 콤보-레인 관계(C24)를 흔드는 게 문제가 되면 이쪽이 대안이다.
...
#   죽음은 '제때 못 잡아 죽는' 쪽이기 때문이다. 밴드는 클리어 한 번의 **처치량**을 올려 그쪽을 친다.
#   실측(10번째 판, 시드 2벌×N=150 · 전부 3패 칸 위):
#     레버 없음   43.0% · 거점사 56.3% · 배치 48.9 · 처치/클리어 2.06
#     밴드 완화   85.0% · 거점사 13.0% · 배치 49.2 · 처치/클리어 2.54 (+23%)
#   ⚠'팡팡'을 동시2·3줄로만 재면 이 레버는 통계에서 사라진다(2.08→2.03). 밴드가 움직이는 건
#     줄 수가 아니라 한 클리어의 처치량이다 — care_probe에 처치/클리어 컬럼을 그래서 넣었다.
#   ⚠전멸 임계는 원래 콤보로 판정한다(_blast_band). 보너스로 전멸이 앞당겨지면 케어가 판당
#     클라이맥스 횟수를 바꿔 연출 위계(C24)를 흔든다.
...
# 밴드 완화 A/B 노브(프로브 전용, dda_enabled와 같은 성격). 게임은 늘 true로 돈다 —
#   같은 칸(3패)에 배급·비행기 완화가 같이 있어서, 밴드의 공은 따로 꺼봐야만 갈린다.
var care_band_enabled: bool = true
...
# 입력 방식. true면 '조각 클릭 → 보드 클릭', false면 잡아끌기.
# 화면 규칙은 두 모드가 동일하다 — 클릭 방식에서도 조각은 커서를 따라오고,
# 못 놓는 자리엔 아무 표시도 하지 않는다(빨간 고스트는 되살리지 않는다).
#
# **PC는 클릭이 기본이다**(2026-08-12). 마우스로 잡아끌기는 버튼을 누른 채 움직여야 해서 손이 더 드는데,
# 얻는 게 없다 — 두 모드의 화면 규칙이 같으니 클릭이 그냥 덜 피곤한 쪽이다. 터치는 반대라
# 모바일은 잡아끌기를 유지한다. **토글은 그대로 남아 있으니 PC서 드래그로 되돌릴 수 있다.**
#
# ⚠기본값을 `show_input_toggle`에 묶은 이유: 그 값이 곧 "여기가 PC인가"이고, 조건을 두 군데 적으면
#   어긋난다. 특히 `SHOOT=1`(영상 촬영)은 **모바일과 같은 화면**을 만들려고 토글을 숨기는 모드라
#   거기선 잡아끌기가 유지돼야 녹화 조작이 폰과 같아진다 — 한 줄로 그것까지 따라온다.
#   (멤버 초기화는 선언 순서대로 도므로 `show_input_toggle`이 위에 있어야 한다. 실측 확인함.)
var click_mode: bool = show_input_toggle
...
#   납품 블록은 180 캔버스에 본체 176(가장자리 투명 2px씩) → 90px 셀에서 간격 2px이 그림에서 나온다.
#   (1차 납품은 여백 6px = 간격 6px이었다. v2에서 좁혔다 — 레퍼런스 실측도 1.7px로 거의 붙어 있다.)
...
# ── 빈 셀 배경(UI_ART_PLAN §4 P0) ──
# 블록과 **짝으로 봐야 판단이 되는** 부품이라 같은 180×180 규격이다. 블록이 칸을 얼마나 채우는지는
#   블록 그림이 정하고(bpad는 코드가 아니라 그림의 여백), 그 밑에 깔리는 판이 이것이다.
# ⚠블록과 달리 **틴트하지 않는다** — 보드 셀은 존 색 전환에서도 원색을 유지하는 '다크 아일랜드'다
#   (넘음 전환은 주변 바·여백만 물들인다). 색을 그림이 갖는다.
# ⚠이 한 장은 **불투명 전면**이다(투명 여백 없음). 칸 사이 홈을 그림이 칠해서 갖고 있다는 뜻 —
#   뚫어 두면 뒤에 오는 것에 따라 격자가 밝아졌다 어두워졌다 하고, 존 색 전환이 있어 실제로 흔들린다.
#   그래서 이웃한 두 칸의 여백이 **합쳐져** 홈이 된다(각 3px → 화면 3px). 블록 간격(2px)보다 넓은 건
#   의도다 — 레퍼런스도 홈 2.7px > 블록 간격 1.7px이다. 생성기 = tools/make_cell_plate.py.
const CELL_TEX_PATH: String = "res://art/cell.png"
var cell_tex: Texture2D = null

# ── 종이비행기(UI_ART_PLAN §4 P1) ──
# 한 장이 세 곳을 덮는다: 보드 픽업(CELL*0.60) · 좌하단 보유 슬롯(90×90의 0.56) · 발사 연출.
#   크기 차가 크므로 진짜 아트가 오면 세 곳 다 눈으로 볼 것(트레이 LOD와 같은 성격).
# ⚠**틴트하지 않는다** — 접힘 명암(두 날개의 밝기 차)이 이 물건의 정체라 2색 이상이다. 해골과 같은 예외.
const PLANE_TEX_PATH: String = "res://art/plane.png"
var plane_tex: Texture2D = null
...
const UI_9S: Dictionary = {
	...
	# 큰 버튼 3종 — 납품본 실측 라운드 반경 22 텍스처px. 마진을 그보다 크게 잡으면 세로 그라데이션의
	#   위/아래가 그만큼 **고정**되고 가운데만 늘어나 이음매에 기울기 꺾임이 생긴다 → 반경 + 여유만.
	"btn_lg":        [32, 32, 32, 32],   # 기본
	"btn_lg_press":  [32, 32, 32, 32],   # 눌림
	"btn_lg_off":    [32, 32, 32, 32],   # 비활성
...
func _init_block_art() -> void:
	...
	if ResourceLoader.exists(CELL_TEX_PATH):
		cell_tex = load(CELL_TEX_PATH) as Texture2D
	if ResourceLoader.exists(PLANE_TEX_PATH):
		plane_tex = load(PLANE_TEX_PATH) as Texture2D
...
const SFX_WORDS: Dictionary = {
	...
	# det = 반복 방지 디튠 폭(비율). ⚠음정이 **의미를 나르는** 단어(clear·chain = 콤보/사다리)는
	#   좁게 준다 — 사다리 한 계단이 200센트인데 디튠이 ±40센트면 계단이 뭉개진다. 반대로 place·tap은
	#   음정에 의미가 없고 가장 자주 반복되므로 넓게(±2.4% ≈ ±41센트) 줘야 기계음 피로가 깨진다.
	# db는 **정규화된 샘플(모두 같은 피크) 기준의 위계**다. 실측 크레스트가 단어마다 달라서
	...
	# ⚠**R4에서 전체를 5~8dB 올렸다.** 유저 판정 "은은하게 출력되는 것 같다" + 레퍼런스 실측이
	...
	#     clear −5.0 / clear2 −11 / chain −11 / place −12 / fail −13 / grab −10 / tap −12 / score −13 dBFS.
	...
	"clear": {"gap": 0.00, "db": -3.6, "det": 0.004},    # 간격 0 = 절대 안 드롭한다(이게 보상이다)
	# ⚠−9.0은 R27에서 −7.6에서 내린 값이다 — 보석 수집음이 연쇄에 묻혀 안 들렸다(§35).
	#   연쇄는 판당 5~9발이라 위계에서 조금 내려도 존재감이 남지만, 수집은 알림이라 안 들리면 0이다.
	"chain": {"gap": 0.03, "db": -9.0, "det": 0.004},
	...
	# ⚠**base +12 · db −7은 R27에서 다시 잡은 값이다**(유저: "수집 사운드가 잘 인지 안 돼").
	#   실측한 원인은 시간 겹침이 아니라 **음색·음역이 `chain`과 같다**는 것이었다: 둘 다 pop_high고
	#   collect의 0·+7이 연쇄 사다리(0/2/4/7/9…)의 계단과 **같은 음**인데다 5.4dB 더 조용했다
	#   → 귀엔 '새 소리'가 아니라 '연쇄가 조금 더 이어진 것'으로 들렸다.
...
const FB_MAP: Dictionary = {
	...
	"score": {"hap": "", "sfx": "score"},     # 점수 롤업 틱 — 초당 십수 발이라 진동은 절대 안 붙인다
	# ⚠fail은 **소리만**. "손실엔 무음" 중 오디오만 유저 요청으로 뒤집었고(2026-07-31), 햅틱은
...
#   왜 리미터인가: 폴리포니 8 + 콤보 아르페지오를 클립 없이 담으려면 보이스마다 헤드룸을 크게
#   비워야 하고, 그러면 **평상시 소리가 너무 작아진다**(R1 실효 −24~−34dBFS). 리미터가 꼭대기만
#   눌러 주면 평균을 모바일 수준으로 올리면서 합산 클리핑을 막을 수 있다.
func _sfx_setup_bus() -> void:
	if AudioServer.get_bus_index("SFX") >= 0:
		return          # 이미 있다 — Main을 두 번 띄우는 하네스에서 버스가 쌓이는 걸 막는다
	var idx: int = AudioServer.bus_count
	AudioServer.add_bus(idx)
	AudioServer.set_bus_name(idx, "SFX")
	AudioServer.set_bus_send(idx, "Master")
	var lim = ClassDB.instantiate("AudioEffectHardLimiter")
	if lim != null:
		lim.set("ceiling_db", -0.5)     # 0dBFS 직전에서 자른다(폰 DAC 인터샘플 여유)
...
func _sfx_init() -> void:
	_sfx_build_bank()
	_sfx_setup_bus()
	var bus_name: String = "SFX" if AudioServer.get_bus_index("SFX") >= 0 else "Master"
...
func _sfx_fire(kind: String, semi: int, db_over: float = 99.0) -> bool:
	...
	_sfx_budget -= 1.0
	_sfx_n += 1
	# 결정적 디튠 — 같은 소리가 연달아 나도 기계음으로 안 들리게. RNG는 안 쓴다.
	#   ⚠R1의 ±0.6%(≈10센트)는 사람 귀에 사실상 안 들려서 반복 피로를 못 막았다. 폭은 단어가
	#   소유한다(SFX_WORDS.det): 음정이 의미를 나르는 clear·chain은 좁게, 반복되는 place·tap은 넓게.
...
func _sfx_step(delta: float) -> void:
	...
	for q in _sfx_queue:
		...
		if _sfx_t >= float(e["at"]):
			_sfx_fire(String(e["kind"]), int(e["semi"]), float(e.get("db", 99.0)))
		else:
			keep.append(e)
	_sfx_queue = keep
...
#   "세로 막대만 계속 나온다"가 됐다. 유도는 1회, 그 뒤는 이 판의 정상 풀(POOL_ONBOARD)로 돌아간다.
...
# 밴드 완화(케어 3패). 완성 줄이 쓸어내는 레인을 +N = 클리어 한 번의 처치량이 는다.
#   전진 완화와 같은 사인(거점사)을 치되, 화면에선 '봐줌'이 아니라 '보상'으로 읽힌다(로켓이 더 나간다).
func _care_band_bonus() -> int:
	if not care_band_enabled:
		return 0
	return CARE_BAND_BONUS if _care_level() >= CARE_BAND_FAILS else 0
...
func _blast_band(rows: Array, cols: Array, combo_val: int) -> Dictionary:
	# 케어(3패)는 여기서 레인을 한 칸 넓힌다 = 같은 클리어가 적을 더 많이 쓴다.
	#   ⚠전멸 임계는 **원래 콤보로** 판정한다(아래 full_board) — 보너스로 전멸이 앞당겨지면
	#   케어가 판당 클라이맥스 횟수를 바꿔 연출 위계(C24)를 흔든다. 밴드가 전 열을 실제로 덮어서
	#   나는 전멸은 그대로 둔다(그건 원래 규칙이다).
	var lanes_n: int = maxi(1, combo_val + _care_band_bonus())
...
func _process(delta: float) -> void:
	...
	if menu_intro >= 0.0:
		if mode == "menu":
			# ⚠delta를 그대로 더하지 않는다. 부팅 직후엔 프레임이 들쭉날쭉해서(실측 17~105ms) 긴 프레임
			#   하나가 이동의 38%를 한 번에 먹는다 — 그게 "뚜둑"이다. 한 프레임이 가져갈 수 있는 몫을
			#   묶어 두면 느린 기기에선 전환이 조금 길어질 뿐, 계단은 늘 같은 폭이 된다.
			menu_intro += minf(delta, 1.0 / 30.0)
	...
	if endless_score_shown < float(endless_score):
		...
		_fb("score")   # 카운터 틱 — 간격(0.055s)이 발화율을 잡으므로 롤업 속도와 무관하게 안정적이다
...
func _draw() -> void:
	...
	if mode == "logo":
		...
		_draw_bg_glow()
		# ⚠홈 버튼을 여기서 미리 한 번 그려 배경으로 덮는다 = **워밍업**(화면엔 안 남는다 — 배경이
		#   불투명하고 홈과 같은 그림이다). 첫 버튼 그리기가 9-slice 텍스처 업로드·셰이더 컴파일을 끌고
		#   오는데, 그게 미끄러지는 도중에 터지면 **그 프레임만 105ms**가 되어 로고가 한 칸 건너뛴다
		#   (실측: 스파이크 시각이 버튼 등장 시각과 정확히 겹쳤다). 여기서 미리 치르면 홈은 이미 더운 상태다.
...
func _draw_select(fnt: Font) -> void:
	...
	#     `_draw_menu` 주석), 홈에서만 들어오는 화면이라 이름을 두 번 말할 자리가 아니다.
	#   ⚠'Cleared N / M'도 되살리지 말 것: **완성률은 라이브 서비스와 싸운다** — 스테이지를 추가할 때마다
	#     분모가 커져 플레이어의 숫자가 저절로 나빠진다(9/13 → 9/18). 남은 판 수는 자물쇠 타일이 이미
	#     말하고 있어 중복이기도 했다. 레퍼런스(Block Blast 어드벤처)도 화면에 숫자가 CTA 하나뿐이다.
	#   제목 = 이 화면을 연 허브 버튼과 **같은 낱말**(adv_big) — 새 문자열을 만들면 둘이 갈린다.
	#   자리는 좌상단 '홈'과 같은 띠(레퍼런스 조판: 뒤로 화살표 ← 제목 → 우측 여백). BACK_BTN에서
	#     계산하므로 버튼을 옮기면 제목이 따라온다.
	var title: String = _t("adv_big")
	var tfs: int = 30
...
# 상단 목표 카드 한 장 = 다크 본체 + accent 테두리. 동사 넷(처치·수집·방어·보스)이 **같은 부품**을 쓰고
#   accent만 갈린다 — 패널/강조바와 같은 구조라 나누는 선도 같은 자리에 긋는다.
func _draw_card(r: Rect2, accent: Color) -> void:
	if ui_9s.has("card"):
		_blit_9s("card", r)              # 본체는 틴트 안 함(다크 톤을 그림이 갖는다)
		_blit_9s("card_edge", r, accent) # 테만 그린 같은 캔버스 레이어 → 라운드가 정의상 일치
		return
...
func _draw_plane_icon(center: Vector2, s: float) -> void:
	if plane_tex != null:
		# 도형 시절의 실제 폭이 s와 거의 같다(날개끝 ±0.52s) → 한 변 s의 정사각에 그린다.
		#   여백을 얼마나 둘지는 180×180 안에서 그림이 정한다(블록·적과 같은 규약).
		draw_texture_rect(plane_tex, Rect2(center - Vector2(s, s) * 0.5, Vector2(s, s)), false)
		return
	var h: float = s * 0.5
...
# 물건이 앉는 자리 = 판 + 테두리. **트레이 3칸과 비행기 보유 슬롯이 같은 부품**을 쓴다 —
...
# 빈 셀 한 칸 = 판 + 격자선. 블록이 없는 자리는 전부 여기를 지난다(보드 64칸의 배경).
#   ⚠텍스처가 오면 격자선을 **코드가 안 그린다** — 칸 사이 선은 그림의 여백이 만든다(블록과 같은 규약).
#     둘 다 그리면 그림의 라운드 밖으로 각진 선이 겹쳐 지저분해진다(강조바에서 배운 것과 같은 종류).
func _blit_cell(r: Rect2) -> void:
	if cell_tex == null:
		draw_rect(r, C_CELL)   # 8×8 보드 셀은 원색 유지 — 넘음 전환은 주변(바·여백)만, 보드는 다크 아일랜드
		draw_rect(r, C_GRID, false)
		return
	draw_texture_rect(cell_tex, r, false)

const RIM_PX: float = 1.0   # 블록 뒤로 삐져나오는 테 두께. 블록 여백(그림 1px)과 같게 = 옆 칸 안 침범
...
func _draw_board(fnt: Font) -> void:
	...
	if ui_9s.has("board_frame"):
		...
		if endless_beat_best:
			var gp2: float = 0.5 + 0.5 * sin(anim_t * 2.2)
	...
	for r in range(ROWS):
		for c in range(COLS):
			...
			_blit_cell(Rect2(rx, ry, CELL, CELL))
...
func _draw_core(fnt: Font) -> void:
	...
	# 텍스처 경로에선 하이라이트·테두리를 **안 그린다**(버튼과 같은 규약 — 그림이 이미 갖고 있다).
	var bar_tex: bool = ui_9s.has("hp_track") and ui_9s.has("hp_fill")
	var track_r: Rect2 = Rect2(sx, sy, sw, strip_h)
	var fill_col: Color = Color(0.86, 0.24, 0.20).lerp(Color(0.28, 0.82, 0.45), ratio)
	if bar_tex:
		_blit_9s("hp_track", track_r)
		_blit_9s("hp_fill", Rect2(sx, sy, sw * ratio, strip_h), fill_col)
	else:
		draw_rect(track_r, Color(0.08, 0.03, 0.04))
		draw_rect(Rect2(sx, sy, sw * ratio, strip_h), fill_col)
	# 곧-깎일 구간(토큰 비행 중): [논리 core_hp .. 표시 core_hp_vis]를 붉게 펄스 = "여기가 곧 부서진다" 예고.
	#   채움과 오른쪽 끝을 공유하므로 **채움과 같은 부품**으로 그린다 — 각진 사각을 얹으면 둥근 끝 밖으로 삐져나온다.
	if dmg_flights.size() > 0:
		var warn_lo: float = clampf(float(core_hp) / float(core_max), 0.0, 1.0)
		var wp: float = 0.35 + 0.35 * sin(anim_t * 9.0)
		var warn_r: Rect2 = Rect2(sx + sw * warn_lo, sy, sw * (ratio - warn_lo), strip_h)
		if not (bar_tex and _blit_9s("hp_fill", warn_r, Color(1.0, 0.4, 0.3, wp))):
			draw_rect(warn_r, Color(1.0, 0.4, 0.3, wp))
	if not bar_tex:
		# 상단 하이라이트(입체감)
		draw_rect(Rect2(sx, sy, sw * ratio, strip_h * 0.4), Color(1.0, 1.0, 1.0, 0.18))
		draw_rect(track_r, Color(1.0, 1.0, 1.0, 0.55), false, 2.0)
...
func _draw_bottom(fnt: Font) -> void:
	...
	if _plane_allowed():
		...
		if plane_held:
			...
			if plane_armed:
				...
				_draw_slot_box(pr, Color(0.16, 0.24, 0.30),
						Color(C_PLANE_GLOW.r, C_PLANE_GLOW.g, C_PLANE_GLOW.b, 0.55 + 0.40 * hp2), 2.5)
			...
			_draw_slot_box(pr, Color(0.10, 0.10, 0.16), Color(0.30, 0.34, 0.42, 0.5), 1.5)
	...
	for i in range(3):
		...
		_draw_slot_box(sr, bg_col, Color(0.35, 0.35, 0.5, 0.6), 1.5)
