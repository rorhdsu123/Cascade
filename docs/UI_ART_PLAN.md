...
## 📦 지금 어디까지 왔나 (2026-08-08)

**P0·P1이 전부 들어왔습니다. 쓸 수 있는 것은 전량 붙였습니다.**

8/8 납품분 22장 중 **16장을 그대로**, **2장을 손봐서** 적용했습니다. 남은 4장은 아래 "못 쓴 것"입니다.

| 적용된 것 | 받은 날 | 비고 |
|---|---|---|
| 블록 · 빈 셀 · 패널 · 강조바 · 큰 버튼 3상태 · 적 basic | 8/4~8/5 | 이전 회차. 그대로 |
| **아이콘 8종** | 8/8 | 흰 단색으로 잘 왔습니다. 코드가 자리마다 색을 입힙니다 |
| **종이비행기** | 8/8 | 보드 픽업 · 보유 슬롯 · 발사체 3곳 |
| **적 swarm · fast** | 8/8 | 코드제 임시본을 대체 |
| **작은 버튼 2상태 · 고스트 버튼** | 8/8 | 설정 모달 · 뒤로가기 · 결과 팝업 홈 |
| **트레이 슬롯 · 보드 프레임 · HP바 2장 · 목표 카드 · 튜토리얼 말풍선 2색** | 8/8 | 아래 ⚠ 참조 |

### ⚠ 두 장은 그대로 못 써서 개발이 손봤습니다 — **다음 회차엔 회색조로**

**규칙은 하나입니다: 게임이 색을 바꾸는 자리는 회색조로 주셔야 합니다.** 코드가 색을 **곱하기**로 입히기 때문에, 그림이 이미 색을 갖고 있으면 그 위에 다른 색을 못 얹습니다(곱하기는 늘 어두워지기만 합니다).

| 파일 | 무엇이 문제였나 | 어떻게 했나 |
|---|---|---|
| HP바 채움 | 초록으로 주셨는데, 이 띠는 체력이 줄면 **빨강으로 물들어야** 합니다. 초록 위에 빨강을 곱하면 진흙색이 됩니다 | 밝기만 남기고 회색조로 되돌렸습니다. **꽉 찬 상태는 주신 초록 그대로** 나옵니다 |
| 보드 외곽 프레임 | 청색으로 주셨는데, 무한 모드에서 신기록을 갱신하는 동안엔 이 테가 **금색**이 됩니다. 금색×청색 = 탁한 연두가 됐습니다 | 같은 처리. 평상시 청색은 코드가 입힙니다 |

**목표 카드는 테가 본체에 구워져 있었습니다.** 이 카드의 테 색은 "이번 판이 무슨 판인가"를 말하는 신호라(처치=금색 / 수집=젬색 / 방어=보라), 한 장으로 오면 모든 판이 처치판처럼 보입니다. 개발이 테만 떼어내 레이어로 분리했습니다 — 다음엔 **본체 1장 + 테만 남긴 회색조 1장**으로 주세요(강조바와 같은 규칙, §4).

### 못 쓴 것 4종 — **다시 그리지 마세요**

| 파일 | 왜 |
|---|---|
| 스테이지 타일 4종 · 현재 위치 표식 · 별 | **진행 화면이 바뀌었습니다.** 번호 타일 격자를 버리고 **성이 한 층씩 올라가는 그림**이 됐습니다(8/7). 타일이 들어갈 자리가 없어졌습니다. 의뢰서 P1 목록에 남아 있던 항목이라 그리신 건데, 문서를 늦게 고친 개발 잘못입니다 — 죄송합니다 |
| 뒤로가기 버튼 2상태 | 화살표가 그림에 **구워져** 왔는데, 이 버튼은 화살표 + `Home` **글자**가 함께 들어가는 가로로 긴 버튼입니다(132×54). 정사각형 아이콘 버튼과 형태가 다릅니다. 같은 자리는 이번에 주신 **작은 버튼**으로 채웠습니다 |
| 말풍선 꼬리 2종 | 튜토리얼 말풍선은 화면 상단에 가로로 눕는 판이라 꼬리가 붙는 구조가 아닙니다. 꼬리가 달린 말풍선은 **적 머리에 붙는 다른 말풍선**이고, 그건 어두운 판+금색 테라 색이 안 맞습니다 |

⚠**튜토리얼 말풍선은 2색을 판 두 장으로 주셨습니다**(노랑·붉은색). 원래 요청은 "본체 1장 + 테만 남긴 회색조 1장"이었는데, 주신 그림이 알약 전체가 물드는 형태라 테만 떼어낼 데가 없었습니다. **주신 대로 두 장을 갈아끼우는 방식으로 배선을 고쳤습니다** — 이건 그대로 갑니다. 대신 판이 밝아져서 판 위 글자를 어두운 색으로 뒤집었습니다(패널 때와 같은 처리).
...
P1: `card` · `card_edge` · `hp_track` · `hp_fill` · `slot` · `board_frame` · `tut_bubble` · `tut_bubble_warn`
(~~`tile`·`tile_front`·`tile_off`~~ **삭제** — 진행 화면이 성으로 바뀌어 타일이 없어졌다. 위 "못 쓴 것" 참조)
...
- **P1 잔여 납품분 전량 적용(C161, 2026-08-08)** — 22장 중 16장 그대로 · 2장 회색조 복원 · 1장 레이어 분리 · 4장 사용처 없음. 옮기는 표는 `tools/import_art_delivery.py`에 있다(다음 회차에 손으로 다시 복사하지 말 것).
  - ⚠**납품 배율이 부품마다 달랐다.** §5는 2배가 원칙인데 **화면 크기가 이미 정해진 부품**(목표 카드 310×104 · 말풍선 높이 36)은 1:1로, HP바는 520×40으로 왔다. `UI_TEX_SCALE` 하나로 나누면 모서리만 절반 크기로 그려져 라운드가 찌그러진다(가운데는 늘어나므로 안 티난다 = **눈으로 못 잡는 종류**) → `UI_TEX_SCALE_OVR` 표를 팠다.
  - ⚠**`modulate`는 덧칠이 아니라 곱셈이라, 채색본 위에 한 겹 더 그려도 색이 안 바뀐다.** 청색 프레임에 금색을 0.92 알파로 얹어 봤더니 탁한 연두가 나왔다(실측). 색이 갈리는 채널은 **그림을 회색조로 되돌리는 것 말고 방법이 없다**.
  - **슬롯은 반대로 갔다.** 판이 색을 갖고 온 건 같지만 여긴 상태색이 전부 어두워서(빈 슬롯 0.10) 곱하면 네 상태가 다 새까매진다 → **판은 틴트 없이 그리고 상태색을 알파 0.45로 덧칠**한다. 알파 합성이라야 조준(밝은 청록)이 실제로 밝아진다.
  - ⚠**테를 '키워서 뒤에 깔' 때 그림의 투명 여백을 더해야 한다.** 슬롯 납품본은 240×200 캔버스에 여백이 8px(화면 4px) 있어서, 키운 판의 잉크가 앞판 잉크와 같은 자리에 떨어져 **테가 한 픽셀도 안 보였다**(`SLOT_ART_MARGIN`).
  - ⚠**아이콘은 캔버스 대비 잉크가 54~75%였다.** 호출부는 "도형이 뻗는 폭"을 넘기는 규약이라(`체크는 c.x±s로 벌어진다`) 그대로 그리면 아이콘만 25~46% 작아진다 → 로드할 때 알파 바운딩 박스를 재서 나눠 준다(`icon_fill`). ⚠`get_image()`는 `compress/mode=0`(무손실)이라 기기에서도 된다 — VRAM 압축으로 바꾸면 깨진다.
  - ⚠**해골 주석의 전제가 깨졌다.** "뼈색+어두운 눈 2색이라 틴트를 못 건다"고 적어 뒀는데 납품본은 **눈·코가 뚫린 흰 단색 마스크**로 왔다 → 밝은 판 위에선 구멍으로 크림이 비쳐 실루엣이 사라진다. 단색이니 이제 통째로 어둡게 틴트하면 된다(C142의 rim 장치는 텍스처 경로에서 걷어냈다).
  - ⚠**HP바 피격 펄스는 채움으로 그린다.** 트랙은 어두운 색을 그림이 갖고 있어 밝은 붉은색을 곱해도 안 밝아진다 = 펄스가 아예 안 보였다. 두 장이 같은 알약이라 모양은 같다.
