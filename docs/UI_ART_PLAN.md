...
> 작성 2026-08-01 · 개정 2026-08-03(블록 방향 확정 반영). 이 문서가 UI 아트 트랙의 정본이다.
...
- **`art/enemies/basic.png` 임시본 커밋됨(C137, 생성기 `tools/make_basic_placeholder.py`)** — 블록·빈 셀과 같은 급의 '최후 수단'이다. **P0 중 자리지킴이가 없는 유일한 항목이었다**: 블록은 라운드+베벨+광택인데 적만 평면 원이라 짝이 깨져 있었고, 노출은 최대다(st1 적의 100%). 덮어쓰면 갈아끼워지고 지우면 도형 렌더로 되돌아간다(코드 무수정).
