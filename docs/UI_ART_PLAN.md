...
| | 날짜 | 내용 |
|---|---|---|
| **P0 납품** | **8/4 (화)** | 블록 · 패널 · 주버튼 — 이것만 바뀌어도 화면 인상이 갈린다 |
...
- 좌하단: **종이비행기 보유 슬롯**(90×90). 캡처엔 안 보이지만 게임 중 보드에 픽업으로 떨어지고, 집으면 이 슬롯에 들어가며 탭 두 번(조준→발사)으로 쓴다. **세상에 한 대뿐인 특별한 물건**이라는 인상이 필요하다
...
**적 7종과 연출 효과**(줄 터짐 · 폭죽 · 거점 붕괴 · 콤보 텍스트 등)는 제외한다. 매 프레임 값이 바뀌며 절차적으로 생성·애니메이션되는 요소라 정적 이미지로 대체되지 않는다. 별도 회차가 맞다.
...
| 날짜 | 디자이너 | 개발 (Claude) |
|---|---|---|
| **8/2 (일)** | **§7-1 방향 결정**(블록을 기하학이냐 질감이냐) + 형식 합의 → 블록 착수 | 붙일 자리 준비(임포트 설정·틴트 파이프라인·가산 블렌드 패스) |
| **8/3 (월)** | 블록 1차 시안 | 받는 즉시 배선 → **확대 스크린샷 회신** |
| **8/4 (화)** | 🔴 **P0 납품** — 블록 확정 + 패널 + 큰 버튼 | 블록·패널·버튼 배선 |
| **8/5 (수)** | P1 착수 — 아이콘 8종 + 비행기 | 결과 팝업·설정 화면 배선 + 검수 회신 |
...
- **§7-1 답이 늦어지면 전부 밀린다.** 블록 방향은 오늘 안에 정해야 한다.
...
### 7-1. 블록을 어떤 물체로 볼 것인가 ← 🔴 **오늘(8/2) 안에 답이 필요**

- **(가) 기하학적 형태** — 라운드 사각 + 베벨 + 은은한 광택. 깔끔하고 스킨 확장이 쉽다
- **(나) 질감 있는 물체** — 젤리·캔디·나무·유리 등. 표현 상한이 높지만 6색 전개와 스킨 비용이 커진다

이 답에 따라 개발 쪽 구현 방식이 갈린다((가)는 코드로도 가능, (나)는 이미지 필수). 어느 쪽이든 대응한다.

**6일 일정에서는 이게 임계경로다.** 여기서 하루 밀리면 P0 전체가 밀린다. 완벽한 답보다 빠른 답이 낫다.
...
- **ETC2 압축**: `project.godot`의 `import_etc2_astc=true`는 안드로이드 export의 전제조건이라 끌 수 없다. 지금은 텍스처가 없어서 무해했지만 **블록 이미지를 넣는 순간 손실 압축이 실제로 걸린다** — 채도 높은 평면 색은 밴딩이 잘 보이는 조합이다. 블록/UI 아틀라스는 **무손실 임포트로 개별 설정**할 것.
