...
| 파라미터 | 예 | 왜 |
|---|---|---|
| `build_version` | `"0.9.0-L1.1"` | 코호트 분리 — **루프를 값에 담는다**(아래 ⚠) |
...
⚠**`build_version`은 `project.godot`의 `application/config/version`을 그대로 읽는다.
비어 있으면 `0.0.0-dev`로 나간다** — 2026-08-12까지 실제로 그 상태였고, 그동안 찍힌 이벤트는
전부 같은 값이라 **어느 빌드에서 왔는지 가릴 수 없다.**

형식은 `<버전>-L<루프>.<그 루프의 몇 번째 배포>`다 — 예: `0.9.0-L1.1`.
루프를 값 안에 담는 이유는 **시트를 열자마자 어느 루프인지 보이게** 하기 위해서다.
숫자만 올리면 "0.9.4가 루프 몇이었지"를 매번 대조표에서 찾아야 한다.

⚠**웹을 다시 올릴 때마다 끝자리를 올린다.** 루프 안에서 처방을 적용하고 재배포했는데 값이 그대로면
처방 전후가 한 덩어리로 섞여 **"고친 게 먹었나"를 못 읽는다** — 그게 루프 2·3의 유일한 질문이다.
날짜로 가르면 될 것 같지만 안 된다: 브라우저가 옛 판을 캐시해 두면 새 기간에 옛 빌드를 도는
사람이 생기고, **그 어긋남이 하필 처방 효과를 재는 자리에서 일어난다.**
...
| `stage_cleared` | `stage_id`, `goal_type`, `duration_ms`, `max_combo`, `dda_assist_count`, `care_level`, `multi2`, `multi3` | 스테이지별 클리어율·구제(DDA) 의존도 |
| `stage_failed` | `stage_id`, `cause`, `attempt_n`, `care_level`, `multi2`, `multi3` | **스테이지별 벽**(어디서 이탈·재도전 몇 번) |
...
`multi2`/`multi3` = 한 배치로 2줄·3줄 이상을 **동시에** 지운 횟수. `max_combo`(연속 배치로
이어감)와 **다른 축**이고, 유저가 말하는 '팡팡 터지는 맛'은 이쪽이다. 봇 프로브엔 있었는데
실플레이 로그엔 없어서 "케어를 켰는데 화면에서 실제로 싹 터졌나"를 확인할 수단이 없었다
(2026-08-05 S6 판정 때 막힘). 콤보만 보면 케어가 잘 도는 것처럼 보이는데 체감은 아닐 수 있다.
