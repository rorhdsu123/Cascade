...
⚠**인자 없이 돌리면 커밋된 `basic.png`·`swarm.png`을 덮어쓴다.** 그건 출고용 자리지킴이(`make_enemy_master.py`,
C137·C138)고 이 파일이 만드는 건 배선 확인용 그림이라, 덮으면 화면이 눈·꼬리 달린 검사용 도형으로 나간다.
배선만 다시 볼 거면 **출력 디렉터리를 따로 주고**(예: `/tmp/chk`) 거기서 확인할 것.
