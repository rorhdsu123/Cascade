...
#   실행: python3 tools/sfx_prep.py <입력파일> <출력.wav> [--drive N] [--tail-db N] [--gain-db N]
#
...
def prep(src, dst, drive=0.0, tail_db=-40.0, gain_db=0.0, peak=0.97):
    x, rate = load(src)
    ...
    y = fade_out([v * g for v in y], rate)
    save(dst, y, rate)
...
if __name__ == "__main__":
    ...
    prep(a[0], a[1], drive=opt("--drive", 0.0), tail_db=opt("--tail-db", -40.0), gain_db=opt("--gain-db", 0.0))
