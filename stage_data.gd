...
#     막힘사 34 → 26 → 11 → 9.
#   ⚠**2단(2번째 시도)이 +5pt로 거의 안 받친다** — CARE_CLEAR_FAILS=2·CARE_BAND_FAILS=3이라
#     실제 장치는 3·4번째 시도에 걸려 있고, 그 +5pt는 케어가 아니라 DDA의 연패 항이 낸 것이다.
#     즉 **약한 플레이어의 절반(50.1%)이 아무 도움 없이 두 번 연속 진다.** 고칠지는 설계 판단이다 —
#     앞당기면 "겨우 깬 느낌"을 사고(C110 교훈), 두면 초반 이탈을 산다.
