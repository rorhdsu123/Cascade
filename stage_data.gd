...
# 기준 ⑬ **campaign_probe의 수치는 "DDA·케어까지 끈 순수 난이도"다 — 플레이어가 겪는 값이 아니다**(S32).
#   실제 게임은 dda_enabled=true가 기본이고 케어가 연패 수로 그 위에 얹힌다. 프로브는 예전부터
#   dda_enabled=false로 돌았고(의도된 설계다 — 순수 난이도를 재려고), 그래서 이 파일의 기준 ⑨·⑫를
#   포함한 **모든 승률이 그물 없는 값**이다. 목표를 세울 때 이걸 플레이어 경험으로 읽지 말 것.
